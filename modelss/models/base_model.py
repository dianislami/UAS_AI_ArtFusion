        ...
        for name in self.model_names:
            if isinstance(name, str):
                ...
                if not self.isTrain or opt.continue_train:
                    ...
                    # Load state dict leniently to allow minor key differences between
                    # checkpoint and current network definitions (e.g., batchnorm vs instancenorm
                    # naming/ordering). Print missing/unexpected keys for debugging.
                    load_res = net.load_state_dict(state_dict, strict=False)
                    try:
                        missing = load_res.missing_keys
                        unexpected = load_res.unexpected_keys
                        if missing or unexpected:
                            print(f"State dict load warning - missing keys: {missing}")
                            print(f"State dict load warning - unexpected keys: {unexpected}")
                    except Exception:
                        # older/newer PyTorch may return different types; ignore if unavailable
                        pass
